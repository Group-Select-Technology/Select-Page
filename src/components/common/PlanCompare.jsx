...
const PlanCompare = ({ plans, features, tipo }) => {
  ...
  const renderValue = (value) => {
    if (value === true) {
      return <CheckIcon className="w-5 h-5 text-[#2ec6df] mx-auto" />
    }
    if (value === false) {
      return <XMarkIcon className="w-5 h-5 text-gray-400 mx-auto" />
    }
    // Es texto o número
    return <span className="text-gray-600 italic">{value}</span>
  }
  ...
  const getHeaderBg = (index) => {
    if (index === 1) {
      return tipo === "farma" 
        ...
        : "bg-gradient-to-b from-[#17359d] to-[#2ec6df]"
    }
    return "bg-[#f3faff]"
  }
  ...
  const getButtonStyle = (index) => {
    if (index === 1) {
      return "bg-white text-[#2ec6df] border-2 border-white hover:bg-gray-50"
    }
    return tipo === "farma"
      ? "bg-[#2ec6df] text-white hover:bg-[#25b0c8]"
      : "bg-[#2ec6df] text-white hover:bg-[#25b0c8]"
  }
    ...
    <div className="w-full">
          ...
          <thead>
            <tr>
              <th className="p-4 text-left bg-white min-w-[200px]">
                <span className={`font-bold text-lg ${tipo === "farma" ? "text-[#1d98b7]" : "text-[#17359d]"}`}>
                  Categorías
                </span>
              </th>
              {plans.map((plan, index) => (
                <th 
                  key={plan.id} 
                  className={`p-6 text-center min-w-[180px] ${getHeaderBg(index)} ${index === 1 ? "rounded-t-3xl" : ""}`}
                >
                  <h3 className={`text-base font-bold uppercase mb-4 tracking-wide ${getTextColor(index)}`}>
                    {plan.name}
                  </h3>
                  <button className={`px-6 py-2 rounded-full text-xs font-bold transition-colors uppercase tracking-wider ${getButtonStyle(index)}`}>
                    Comenzar
                  </button>
                </th>
              ))}
      ...
      <div className="lg:hidden">
        ...
        <div className={`p-6 text-center rounded-t-2xl ${getHeaderBg(selectedPlan)}`}>
          ...
          <button className={`px-8 py-2.5 rounded-full text-sm font-bold transition-colors uppercase ${getButtonStyle(selectedPlan)}`}>
            Comenzar
          </button>
        </div>
